...
[Materials]
  ...
  #   expression= 'K_pre/(int_width/2) * exp(-Q/(k_Boltz*T))-K2 *(2200-T)'
  ...
  [K_params]
    ...
    prop_names  = 'K_pre                                            Q                                k_Boltz      K_tol     K2'
    prop_values = '${fparse 8.08489251e-07 *(1e6)^3*to/(Av*lo^3)}   ${fparse -1.75530415e-01 *ev/Av}  8.6173e-5    1e-4     ${fparse 3.17587744e-10*(1e6)^3*to/(Av*lo^3)}' 
  ...
  [phase_mobility]
    ...
    constant_expressions  = '${fparse 9.6114e3*eo*(1e-1)/(lo)}'   # s143c4b02.1524e-0500'  ((lo/(2.1524e-04))^3)
...
#------------------------------------------------------------------------------#
[Outputs]
  file_base = ./boxresults/NewSimpleBoxTest_step2
